...
const selectOption = async (page, selector, value) => {
  try {
    ...
    console.error(`Error al seleccionar opción en '${selector}': ${error.message}`);
  }
};
  ...
  try {
    ...
    if (operationType === 'compra' || operationType === 'ambos') {
        ...
        page.on('response', async (response) => {
          if (response.url().includes('https://www4.sii.cl/consdcvinternetui/services/data/facadeService/getDetalleCompraExport')) {
              ...
              const compraFolderPath = path.join(__dirname, `download/${userClean}/compras`);
      ...
      if (operationType === 'venta' || operationType === 'ambos') {
        ...
        const response = await page.waitForResponse(response => {
          return response.url().includes('https://www4.sii.cl/consdcvinternetui/services/data/facadeService/getDetalleVentaExport');
        });
      ...
      const ventaFolderPath = path.join(__dirname, `download/${userClean}/ventas`);
      const ventaFileName = `${year}-${month}.json`;
